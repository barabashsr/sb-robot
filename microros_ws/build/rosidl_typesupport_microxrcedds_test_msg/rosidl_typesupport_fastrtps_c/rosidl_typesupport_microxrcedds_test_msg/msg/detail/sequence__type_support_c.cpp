  ...
  void * untyped_ros_message)
{
  ...
  // Field name: sequence_string_test
  {
    ...
    if (!rosidl_runtime_c__String__Sequence__init(&ros_message->sequence_string_test, size)) {
      fprintf(stderr, "failed to create array for field 'sequence_string_test'");
      return false;
    }
    auto array_ptr = ros_message->sequence_string_test.data;
    for (size_t i = 0; i < size; ++i) {
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = rosidl_typesupport_microxrcedds_test_msg__msg__Sequence;
    is_plain =
      (
...
static size_t _Sequence__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_rosidl_typesupport_microxrcedds_test_msg__msg__Sequence(
