  ...
  void * untyped_ros_message)
{
  ...
  // Field name: int64_sequence
  {
    ...
    if (!rosidl_runtime_c__int64__Sequence__init(&ros_message->int64_sequence, size)) {
      fprintf(stderr, "failed to create array for field 'int64_sequence'");
      return false;
    }
    auto array_ptr = ros_message->int64_sequence.data;
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = rosidl_typesupport_microxrcedds_test_msg__msg__NotEnoughtMemoryTest;
    is_plain =
      (
      offsetof(DataType, end_byte) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _NotEnoughtMemoryTest__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_rosidl_typesupport_microxrcedds_test_msg__msg__NotEnoughtMemoryTest(
