...
MotorController::MotorController(
    ...
    // Initialize PID parameters with adjusted values
    Kp = 20.0;
    Ki = 0.5;
    Kd = 2.0;
    Ko = 1.5;
    safetyDelay = 50; // Reduced from 500ms to 50ms
    moving = false;
    nextPID = 0;
    lastMotorCommand = 0;
}
...
void MotorController::setMotorSpeed(MotorPins& motor, int pwmValue) {
    ...
    // Reduce minimum PWM threshold
    if (abs(pwmValue) < 40 && abs(pwmValue) > 0) {
        pwmValue = (pwmValue > 0) ? 40 : -40;
    }
    ...
    int mappedPWM = mapSpeed(pwmValue);
    ledcWrite(motor.pwmChannel, abs(mappedPWM));
}

void MotorController::updateMotorPID(MotorPins& motor) {
    // Handle safety delay for motor start
    ...
    float feedForward = motor.targetSpeed * 0.5; // Add feed-forward term
    // Calculate output with scaling and minimum threshold
...
int MotorController::mapSpeed(float speed) {
    float maxSpeed = 30.0; // Adjust based on your motor's max speed
